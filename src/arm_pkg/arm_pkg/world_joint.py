...
def world_to_joint(xyz, seed=None):
    """
    世界坐标 → 关节角度.  6DOF 完整 IK, 仅 J6 (夹爪) 锁定为 0°.

    J5 在 IK 中保持自由以保证最高精度, 发布时由调用方归零消除腕部扭转.
    (J5 对末端位置影响极小, 归零不会降低定位精度.)

    参数:
        xyz:  (x, y, z) 现实世界坐标, 单位米, tuple/list/array 均可
        seed: 上一次的关节角度 (度), 作为 IK 搜索起点,
              使相邻帧的解保持一致, 避免多解振荡.
    ...
    用法:
        from world_joint import world_to_joint
        angles = world_to_joint((0.15, 0.20, 0.25))
        angles = world_to_joint((0.16, 0.21, 0.25), seed=angles)
    """
    ik = _get_ik()
    ...
    try:
        ...
        return ik.ik((x_urdf, y_urdf, z_urdf), degrees=True, seed=seed)
    finally:
        # 恢复 J6 原始限位
